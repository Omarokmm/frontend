  ...
  useEffect(()=>{
    ...
    .then((res) => {
      ...
      if(userData.isAdmin ? userData.departments[0].name === "CadCam" : departments[0].name === "CadCam"){
        setStartCases(
        result.casesStart.filter(item => 
        !item.isHold && // Ensure isHold is false
        item.cadCam.actions.length > 0 && // Ensure actions array is not empty
        item.cadCam.actions[item.cadCam.actions.length - 1].prfeix === "start" && // Ensure last action's prfeix is "start"
    ...
    if(date != "")
    {
      ...
      if(userData.isAdmin ? userData.departments[0].name === "CadCam" : departments[0].name === "CadCam"){
        const filteredCases = buffCasesStartingUser.filter((item) => {
          return  _global.formatDateToYYYYMMDD(item.cadCam.actions[item.cadCam.actions.length  - 1]?.dateStart) === date;
      });
      setStartCases(filteredCases);
      }
      if(userData.isAdmin ? userData.departments[0].name === "Caramic" : departments[0].name === "Caramic"){
        const filteredCases = buffCasesStartingUser.filter((item) => {
          return  _global.formatDateToYYYYMMDD(item.ceramic.actions[item.ceramic.actions.length  - 1]?.dateStart) === date;
      });
      setStartCases(filteredCases);
      }
      if(userData.isAdmin ? userData.departments[0].name === "Fitting" : departments[0].name === "Fitting"){
        const filteredCases = buffCasesStartingUser.filter((item) => {
          if(item.fitting.actions.length > 0)
          return  _global.formatDateToYYYYMMDD(item.fitting.actions[item.fitting.actions.length  - 1]?.dateStart) === date;
      });
      ...
      if(userData.isAdmin ? userData.departments[0].name === "Plaster" : departments[0].name === "Plaster"){
        const filteredCases = buffCasesStartingUser.filter((item) => {
          return  _global.formatDateToYYYYMMDD(item.plaster.actions[item.plaster.actions.length  - 1]?.dateStart) === date;
      });
      ...
      if(userData.isAdmin ? userData.departments[0].name === "Reception" : departments[0].name === "Reception"){
        const filteredCases = buffCasesStartingUser.filter((item) => {
          return  _global.formatDateToYYYYMMDD(item.receptionPacking.actions[item.receptionPacking.actions.length  - 1]?.dateStart) === date;
      });
      ...
      if(userData.isAdmin ? userData.departments[0].name === "Marketing" : departments[0].name === "Marketing"){
        const filteredCases = buffCasesStartingUser.filter((item) => {
          return  _global.formatDateToYYYYMMDD(item.designing.actions[item.designing.actions.length  - 1]?.dateStart) === date;
      });
      ...
      if(userData.isAdmin ? userData.departments[0].name === "QC" : departments[0].name === "QC"){
        const filteredCases = buffCasesStartingUser.filter((item) => {
          return  _global.formatDateToYYYYMMDD(item.qualityControl.actions[item.qualityControl.actions.length  - 1]?.dateStart) === date;
      });
      ...
      if(userData.isAdmin ? userData.departments[0].name === "Drivers" : departments[0].name === "Drivers"){
        const filteredCases = buffCasesStartingUser.filter((item) => {
          return  _global.formatDateToYYYYMMDD(item.delivering.actions[item.delivering.actions.length  - 1]?.dateStart) === date;
      });
...
const getFinisheingDate = (item) => {
  if (item) {
    ...
    if((userData.isAdmin ? (userData.departments[0].name === "Fitting" && userData.lastName === "Jamous")   : (departments[0].name === "Fitting" && userData.lastName === "Jamous")) ){
...
const getStartingDate = (item) => {
  if (item) {
    let startDateStr = "";
    if (userData.isAdmin ? userData.departments[0].name === "CadCam" : departments[0].name === "CadCam") {
      startDateStr = _global.formatDateToYYYYMMDD(item.cadCam.actions[item.cadCam.actions.length  - 1]?.dateStart);
    }
    if((userData.isAdmin ? (userData.departments[0].name === "Fitting" && userData.lastName === "Jamous")   : (departments[0].name === "Fitting" && userData.lastName === "Jamous")) ){
      console.log("FITTING CAD CAM")  
     startDateStr = _global.formatDateToYYYYMMDD(item.cadCam.actions[item.cadCam.actions.length  - 1]?.dateStart);
    ...
    if (userData.isAdmin ? userData.departments[0].name === "Caramic" : departments[0].name === "Caramic") {
       startDateStr = _global.formatDateToYYYYMMDD(item.ceramic.actions[item.ceramic.actions.length  - 1]?.dateStart);
    }
    if ((userData.isAdmin ? userData.departments[0].name === "Fitting" : departments[0].name === "Fitting") && item.fitting.actions?.length > 0) {
      startDateStr = _global.formatDateToYYYYMMDD(item.fitting.actions[item.fitting.actions.length  - 1]?.dateStart);
    }
    if (userData.isAdmin ? userData.departments[0].name === "Plaster" : departments[0].name === "Plaster") {
      startDateStr = _global.formatDateToYYYYMMDD(item.plaster.actions[item.plaster.actions.length  - 1]?.dateStart);
    }
    if (userData.isAdmin ? userData.departments[0].name === "Reception" : departments[0].name === "Reception") {
      startDateStr = _global.formatDateToYYYYMMDD(item.receptionPacking.actions[item.receptionPacking.actions.length  - 1]?.dateStart);
    }
    if (userData.isAdmin ? userData.departments[0].name === "Marketing" : departments[0].name === "Marketing") {
      startDateStr = _global.formatDateToYYYYMMDD(item.designing.actions[item.designing.actions.length  - 1]?.dateStart);
    }
    if (userData.isAdmin ? userData.departments[0].name === "Drivers" : departments[0].name === "Drivers") {
      startDateStr = _global.formatDateToYYYYMMDD(item.delivering.actions[item.delivering.actions.length  - 1]?.dateStart);
    }
     return startDateStr
  } else {
    return '-';
...
const getHoldingDate = (item) => {
  if (item) {
    let pauseDateStr = "";
    if (userData.isAdmin ? userData.departments[0].name === "CadCam" : departments[0].name === "CadCam") {
      pauseDateStr = _global.formatDateToYYYYMMDD(item?.historyHolding[item.historyHolding.length - 1]?.date);
    }
    if((userData.isAdmin ? (userData.departments[0].name === "Fitting" && userData.lastName === "Jamous")   : (departments[0].name === "Fitting" && userData.lastName === "Jamous")) ){
      pauseDateStr = _global.formatDateToYYYYMMDD(item?.historyHolding[item.historyHolding.length - 1]?.date);
    }
    if (userData.isAdmin ? userData.departments[0].name === "Caramic" : departments[0].name === "Caramic") {
       pauseDateStr = _global.formatDateToYYYYMMDD(item.ceramic.actions[item.ceramic.actions.length  - 1]?.datePause);
    }
    if ((userData.isAdmin ? userData.departments[0].name === "Fitting" : departments[0].name === "Fitting") && item.fitting.actions?.length > 0) {
      pauseDateStr = _global.formatDateToYYYYMMDD(item.fitting.actions[item.fitting.actions.length  - 1]?.datePause);
    }
    if (userData.isAdmin ? userData.departments[0].name === "Plaster" : departments[0].name === "Plaster") {
      pauseDateStr = _global.formatDateToYYYYMMDD(item.plaster.actions[item.plaster.actions.length  - 1]?.datePause);
    }
    if (userData.isAdmin ? userData.departments[0].name === "Reception" : departments[0].name === "Reception") {
      pauseDateStr = _global.formatDateToYYYYMMDD(item.receptionPacking.actions[item.receptionPacking.actions.length  - 1]?.datePause);
    }
    if (userData.isAdmin ? userData.departments[0].name === "Marketing" : departments[0].name === "Marketing") {
      pauseDateStr = _global.formatDateToYYYYMMDD(item.designing.actions[item.designing.actions.length  - 1]?.datePause);
    }
    if (userData.isAdmin ? userData.departments[0].name === "Drivers" : departments[0].name === "Drivers") {
      pauseDateStr = _global.formatDateToYYYYMMDD(item.delivering.actions[item.delivering.actions.length  - 1]?.datePause);
    }
     return pauseDateStr
  } else {
    return '-';
...
const getHoldingreason = (item) => {
  if (item) {
    ...
    if (userData.isAdmin ? userData.departments[0].name === "Caramic" : departments[0].name === "Caramic") {
      reason = item.ceramic.actions[item.ceramic.actions.length  - 1]?.msg.replace(/.*\bbecause\b\s*/, '').trim();
    }
    if ((userData.isAdmin ? userData.departments[0].name === "Fitting" : departments[0].name === "Fitting") && item.fitting.actions?.length > 0) {
      reason = item.fitting.actions[item.fitting.actions.length  - 1]?.msg.replace(/.*\bbecause\b\s*/, '').trim();
    }
    if (userData.isAdmin ? userData.departments[0].name === "Plaster" : departments[0].name === "Plaster") {
      reason = item.plaster.actions[item.plaster.actions.length  - 1]?.msg.replace(/.*\bbecause\b\s*/, '').trim();
    }
    if (userData.isAdmin ? userData.departments[0].name === "Reception" : departments[0].name === "Reception") {
      reason = item.receptionPacking.actions[item.receptionPacking.actions.length  - 1]?.msg.replace(/.*\bbecause\b\s*/, '').trim();
    }
    if (userData.isAdmin ? userData.departments[0].name === "Marketing" : departments[0].name === "Marketing") {
      reason = item.designing.actions[item.designing.actions.length  - 1]?.msg.replace(/.*\bbecause\b\s*/, '').trim();
    }
    if (userData.isAdmin ? userData.departments[0].name === "Drivers" : departments[0].name === "Drivers") {
      reason = item.delivering.actions[item.delivering.actions.length  - 1]?.msg.replace(/.*\bbecause\b\s*/, '').trim();
    }
     return reason
  } else {
    return '-';
  ...
  const handleClick = (text) => {
    ...
    <div className="row">
     ...
     <div class="tab-content" id="myTabContent" onClick={() => {setSearchText("");setSearchTextHold("");setSearchTextStart("")}}>
     <div
      ...
      <div ref={userRef1} style={{width:"100%"}}>
        ...
        <tbody>
            ...
            <td className="bg-success p-2 text-dark bg-opacity-50">
                <b>{
                (sumOfTeethNumbersLength("Start") - getStudyCases(groupCasesTeethNumbersByName("Start")))
                }</b> 
                </td>
            </tr>
            }
            ...
            <td colSpan={5}>
                ...
                {groupCasesTeethNumbersByName("Start")?.map((item)=>
                    <p className="mb-0">
                    <span>{item.name}:</span> 
    ...
    >
      <div className="row">
        ...
        <div className="col-2 mb-3 print-btn" >
              <button className="btn btn-sm btn-primary " onClick={() => {handleClick("Holding Cases");handlePrint2(); }}> <i class="fas fa-print"></i> print</button>
            </div>
        </div> 
    ...
    <div ref={userRef2} style={{width:"100%"}}>
        ...
        <tbody>
            ...
            <td className="bg-success p-2 text-dark bg-opacity-50">
                <b>{
                (sumOfTeethNumbersLength("Pause") - getStudyCases(groupCasesTeethNumbersByName("Pause")))
                }</b> 
                </td>
            </tr>
            }
            ...
            <td colSpan={6}>
                <div className="summary-teeth-cases">
                {groupCasesTeethNumbersByName("Pause")?.map((item)=>
                    <p className="mb-0">
                    <span>{item.name}:</span> 
                    <b className="badge text-bg-success">{item.count}</b>
                </p>
      ...
      >
        ...
        <div className="col-12 mb-3 print-btn" >
          <button className="btn btn-sm btn-primary " onClick={() => {handleClick("Finished Cases");handlePrint()}} > <i class="fas fa-print"></i> print</button>
        </div>
        }
      </div>
        ...
        <tbody>
            {casesUser.map((item) => (
            <tr className="c-pointer" key={item._id} onClick={() => viewCase(item, "view")}>
                <td >
                {item.caseNumber}
                </td>
                <td >
            ...
            <td className="bg-success p-2 text-dark bg-opacity-50">
                <b>{
                (sumOfTeethNumbersLength("End") - getStudyCases(groupCasesTeethNumbersByName("End")))
                }</b> 
                </td>
            </tr>
            }
            ...
            <td colSpan={5}>
                <div className="summary-teeth-cases">
                {groupCasesTeethNumbersByName("End")?.map((item)=>
                    <p className="mb-0">
                    <span>{item.name}:</span> 
                    <b className="badge text-bg-success">{item.count}</b>
                </p>
