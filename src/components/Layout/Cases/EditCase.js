...
const EditCase = () => {
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      const res = await axios.post(`${_global.BASE_URL}cases/upload`, formData, {
      ...
      showToastMessage("Error uploading files", "error");
    } finally {
  ...
  const removePhoto = async (index) => {
    const photoToRemove = caseModel.photos[index];

    // Attempt backend deletion if it has a Cloudinary publicId
    if (photoToRemove && photoToRemove.publicId) {
      try {
        await axios.post(`${_global.BASE_URL}cases/deleteFile`, {
          publicId: photoToRemove.publicId,
          resourceType: photoToRemove.resourceType
        });
      } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    if (dentistObj.id !== "") {
      ...
      let model = {
        ...
        isPhoto: (caseModel.photos && caseModel.photos.length > 0) ? true : false,
        ...
        <div className="card-body">
          <div class="row">
            ...
            <div className={caseModel.isRedo ? "col-lg-6 mt-4" : "col-12 mt-4"}>
              ...
              <div style={{ marginBottom: '50px' }}>
                <ReactQuill
                  ...
                  onChange={(val) => setCaseModel((prev) => ({ ...prev, jobDescription: val }))}
                  style={{ height: '200px' }}
                />
              </div>
            </div>
              ...
              <div className={caseModel.isRedo ? "col-lg-6 mt-4" : "col-12 mt-4"}>
                ...
                <div style={{ marginBottom: '50px' }}>
                  <ReactQuill
                    ...
                    onChange={(val) => setCaseModel((prev) => ({ ...prev, redoReason: val }))}
                    style={{ height: '200px' }}
                  />
                </div>
              </div>
            ...
            <div className="col-lg-12 mb-4 mt-3">
              <label className="form-label fw-bold">Case Attachments (Photos/Videos)</label>
              <div
                className="upload-dropzone border border-dashed rounded p-4 text-center bg-light position-relative"
                style={{ cursor: 'pointer', borderStyle: 'dashed', borderWidth: '2px', borderColor: '#0d6efd', transition: 'all 0.2s' }}
                onClick={() => document.getElementById('case-file-upload').click()}
                onMouseOver={(e) => { e.currentTarget.style.backgroundColor = '#eef2f7'; e.currentTarget.style.borderColor = '#0a58ca'; }}
                onMouseOut={(e) => { e.currentTarget.style.backgroundColor = '#f8f9fa'; e.currentTarget.style.borderColor = '#0d6efd'; }}
              >
                <input
                  id="case-file-upload"
                  type="file"
                ...
                <div className="py-2">
                  ...
                  <p className="mb-1 fw-semibold text-secondary">Click to upload or drag files here</p>
                  <p className="text-muted small mb-0">Supports images and videos (JPG, PNG, MP4, etc.)</p>
                </div>
              </div>
              {isUploading && (
                <div className="d-flex align-items-center gap-2 text-primary mt-2">
                  <span className="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>
                  <span>Uploading files to secure storage...</span>
                </div>
              )}
              {caseModel.photos && caseModel.photos.length > 0 && (
                <div className="mt-4">
                  <label className="form-label text-muted small fw-semibold">Uploaded Files ({caseModel.photos.length})</label>
                  <div className="d-flex flex-wrap gap-3 mt-1">
                    {caseModel.photos.map((photo, index) => (
                      <div key={index} className="position-relative border bg-white rounded shadow-sm overflow-hidden"
                        style={{ width: '130px', height: '130px', transition: 'transform 0.2s', cursor: 'default' }}
                        onMouseOver={(e) => e.currentTarget.style.transform = 'scale(1.02)'}
                        onMouseOut={(e) => e.currentTarget.style.transform = 'none'}
                      >
                        <button
                          type="button"
                          className="btn btn-sm btn-danger position-absolute top-0 end-0 m-1 shadow-sm d-flex align-items-center justify-content-center"
                          style={{ width: '22px', height: '22px', borderRadius: '50%', padding: 0, zIndex: 10, border: '1px solid rgba(255,255,255,0.8)' }}
                        ...
                        {photo && photo.resourceType === 'video' ? (
                          <video src={photo.url} className="w-100 h-100 object-fit-cover" controls muted />
                        ) : (
                          <img src={photo && photo.url ? photo.url : photo} alt="upload preview" className="w-100 h-100 object-fit-cover" />
                        )}
                      </div>
                    ))}
                  </div>
