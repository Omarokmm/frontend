...
const EditCase = () => {
  ...
  console.log(location.state?.isRedo); // true
  const { id } = useParams();
  console.log(id);
  ...
  const [isRedo, setIsRedo] = useState(location.state?.isRedo);
  ...
  const handleSubmit = async () => {
    ...
    if (dentistObj.id !== "" &&  caseModel.redoReason !== "" ) {
      ...
      let model = {
        ...
        isRedo: isRedo,
        oldCaseIds: isRedo
        ? [...new Set([...(caseModel.oldCaseIds || []), id])] // Ensure uniqueness
        : caseModel.oldCaseIds || [], // Keep existing or default to empty array        teethNumbers: teethNumbers,
        ...
        logs: caseModel.logs.push({
          ...
          date: new Date(),
          flag: 'redo',
          msg: `Redo beacuse ${caseModel.naturalOfWork}`,
        }
      ),
    ...
    <div className="content ">
      <div className="card">
        <h5 class="card-title edit-case-title">
          ...
          <span>
            {isRedo? 'Redo' : 'Edit Case'} <b>#{caseModel.caseNumber} </b>
          </span>
        </h5>
