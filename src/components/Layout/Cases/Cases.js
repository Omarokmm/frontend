...
const Cases = () => {
  ...
  const sortData = (data, key, direction) => {
    ...
    return [...data].sort((a, b) => {
      ...
      if (key === "doctorName") {
        ...
        const getCeramicTechName = (item) => {
          if (!item.assignmentHistory || item.assignmentHistory.length === 0) return "";
          for (let i = item.assignmentHistory.length - 1; i >= 0; i--) {
            const historyEntry = item.assignmentHistory[i];
            const assignments = historyEntry.newAssignment;
            if (assignments && Array.isArray(assignments)) {
              const ceramic = assignments.find(a => a.department === "Caramic" || a.department === "Ceramic");
              if (ceramic) return ceramic.userName?.toLowerCase() || "";
            }
          }
          return "";
        };
        aValue = getCeramicTechName(a);
        bValue = getCeramicTechName(b);
      } else {
        return 0;
      }
    ...
    axios
      ...
      .then((res) => {
        ...
        if (isUrgentCase) {
          ...
          setAllCases(prev => updateList(prev));
          setUrgentCases(prev => [result, ...prev]);
          setInProcessCases(prev => updateList(prev));
          setHoldingCases(prev => updateList(prev));
          setStudyCases(prev => updateList(prev));
          setFinishedCases(prev => updateList(prev));
          setRedoCases(prev => updateList(prev));
          setDelayCases(prev => updateList(prev));
          setPackingCases(prev => updateList(prev));
          setForWorkCases(prev => updateList(prev));
          setForAssignCases(prev => updateList(prev));
          setAllCasesImplants(prev => updateList(prev));
          setNotStartCases(prev => updateList(prev));
          ...
          setAllCases(prev => updateList(prev));
          setUrgentCases(prev => prev.filter((user) => user._id !== result._id));
          setInProcessCases(prev => updateList(prev));
          setHoldingCases(prev => updateList(prev));
          setStudyCases(prev => updateList(prev));
          setFinishedCases(prev => updateList(prev));
  ...
  const updateDeadlineFieldsAcrossLists = (targets, payload) => {
    ...
    setStudyCases((prev) => updateStateList(prev));
  };
  ...
  const togglePriority = async (item) => {
    ...
    setInProcessCases(prev => updateList(prev));
    setHoldingCases(prev => updateList(prev));
    setUrgentCases(prev => updateList(prev));
    setStudyCases(prev => updateList(prev));
    setFinishedCases(prev => updateList(prev));
    setRedoCases(prev => updateList(prev));
    setDelayCases(prev => updateList(prev));
    ...
    try {
      ...
      setInProcessCases(prev => revertList(prev));
      setHoldingCases(prev => revertList(prev));
      setUrgentCases(prev => revertList(prev));
      setStudyCases(prev => revertList(prev));
      setFinishedCases(prev => revertList(prev));
      setRedoCases(prev => revertList(prev));
      setDelayCases(prev => revertList(prev));
      setPackingCases(prev => revertList(prev));
      setForWorkCases(prev => revertList(prev));
      setForAssignCases(prev => revertList(prev));
      setAllCasesImplants(prev => revertList(prev));
    }
  };
  ...
  const handleScheduleSave = async () => {
    ...
    try {
      ...
      updateDeadlineFieldsAcrossLists(targets, payload);
  ...
  const handleResetUnschedule = async (item) => {
    ...
    const targets = [item._id];
    const payload = {
      deadlineCadCam: null,
      deadlineFitting: null,
      deadlineCeramic: null,
    };
    ...
    try {
      ...
      showToastMessage("Error resetting schedule", "error");
    }
  };
                      ...
                      >
                        ...
                        <td>
                          <div className="actions-btns">
                            ...
                            {((user.roles[0] === _global.allRoles.technician &&
                              user.lastName === "Jamous") || (user.lastName === "Harrat") ||
                              (user.roles[0] === _global.allRoles.admin &&
                                departments[0].name === "QC") ||
                              user.roles[0] ===
                              _global.allRoles.super_admin) && (
                            ...
                            {(user.roles[0] === _global.allRoles.admin ||
                              user.roles[0] === _global.allRoles.super_admin ||
                              user.roles[0] === _global.allRoles.Reception ||
                              user.roles[0] === _global.allRoles.receptionPacking ||
                ...
                <table className="table table-responsive text-center table-bordered">
                  <thead>
                    <tr className="table-secondary">
                      ...
                      <th scope="col" onClick={() => handleSort("ceramic")} style={{ cursor: "pointer" }}>
                        Ceramic {renderSortIcon("ceramic")}
                      </th>
            ...
            <div key={index} className=" ">
              ...
              >
                ...
                <div className="mb-3 p-2 h-100 border border rounded border-warning-subtle f-18">
                  <b>Notes/ Details: </b>
                  <small className="" dangerouslySetInnerHTML={{ __html: item.jobDescription }}></small>
