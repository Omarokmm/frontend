          ...
          <div class="row">
            ...
            <div className="col-lg-12 mb-4 mt-4">
              ...
              <input
                ...
                accept="image/*,video/*"
              />
              {isUploading && <div className="text-primary mt-2">Uploading files...</div>}
              {caseModel.photos && caseModel.photos.length > 0 && (
                <div className="mt-3 d-flex flex-wrap gap-3">
                  {caseModel.photos.map((photo, index) => (
                    <div key={index} className="position-relative border p-1" style={{ width: '120px', height: '120px' }}>
                      <button type="button" className="btn btn-sm btn-danger position-absolute top-0 end-0 m-1 z-1" style={{ padding: '0 6px', zIndex: 10 }} onClick={() => removePhoto(index)}>&times;</button>
                      {photo && photo.resourceType === 'video' ? (
                        <video src={photo.url} className="w-100 h-100 object-fit-cover" controls muted />
                      ) : (
                        <img src={photo && photo.url ? photo.url : photo} alt="upload" className="w-100 h-100 object-fit-cover" />
                      )}
                    </div>
                  ))}
                </div>
